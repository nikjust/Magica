...
public class MagicaModVariables {
	public static class WorldVariables extends WorldSavedData {
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				MagicaMod.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(1, this));
			} else {
	...
	public static class MapVariables extends WorldSavedData {
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				MagicaMod.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(0, this));
			} else {
	...
	public static class WorldSavedDataSyncMessage {
		...
		private static void syncData(WorldSavedDataSyncMessage message, LogicalSide side, World world) {
			if (side.isServer()) {
				...
				if (message.type == 0) {
					MagicaMod.PACKET_HANDLER.send(PacketDistributor.ALL.noArg(), message);
					world.getServer().getWorld(DimensionType.OVERWORLD).getSavedData().set(message.data);
				} else {
					MagicaMod.PACKET_HANDLER.send(PacketDistributor.DIMENSION.with(world.dimension::getType), message);
